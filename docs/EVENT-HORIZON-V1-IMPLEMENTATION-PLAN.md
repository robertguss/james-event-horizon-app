...
This file is the overnight build authority. If it conflicts with starter-kit
defaults, this file wins for Event Horizon product surfaces. `DESIGN.md` wins
for visual tokens. `docs/PRODUCT-BRIEF.md` wins for product scope.
`docs/missions/mission-01.md` wins for Mission 1 sentence/question/hint text
(copied into the seed JSON below).
...
- If Slice 1 assumed live Clerk / live Convex as the only path, the **next**
  agent immediately adds the fixture adapter (`VITE_EH_DATA=fixture` default) on
  top of whatever Slice 1 landed — do not rip out Slice 1 UI chrome.

**Overnight constraint:** fixtures + tests + screenshots. Not iPad. Not live
Clerk hosted sign-in. Not live Convex cloud required.
...
- Deterministic missions: passage → questions → Check → hints → XP → debrief →
  Level Up
- Tap-the-evidence + multiple choice
- 4-step Socratic hint ladder (Grok server-side optional; **static fallback
  required**)
...
- Fixture-first overnight path so CI and agents run with **no Clerk keys** and
  **no Convex URL**
...
- Replacing Check CTA with Scan as the primary confirm label on mission answer
  row
...
- Teal + gold **framed** cream reading card (`reading-panel` `#FFF8F0`, text
  `#1A1528`)
- Passage ≥22px Nunito, airy line-height
- Mid-screen answer affordances
- Bottom tool row — **five pills** pattern (visual rhythm):  
  **Check (TEAL primary CTA)** · gold accent · teal accent · gold accent ·
  **Hint (GOLD `button-hint`)**
- **Hint is GOLD** per `DESIGN.md` `button-hint` — **not** teal from any Imagine
  bake-off
- **CTA label = Check** (not Scan). Scan may remain metaphor copy elsewhere;
  confirm action is **Check**
- Telescope / evidence reticle prop for locate + evidence taps (soft teal glow,
  not neon)
...
- Gold star / XP celebration language; one motion verb; honor
...
**UI never calls Convex from components.** No `useQuery`/`useMutation` sprinkled
in mission UI — go through the adapter.
...
1. **`VITE_EH_DATA=fixture` is the overnight default** — set in `.env.example` /
   agent docs; do not require unset→fixture gymnastics.
2. **`fixtureAuth` skips live Clerk.** No publishable key required. Fixture
   lands the kid on `/hub`.
3. **Fixture parent PIN:** `1234` (dev-only). Document in this plan + parent
   gate UI helper text in dev builds only.
4. **Mission 1 fixture module:** `lib/eh/fixtures/mission01.ts` — full seed from
   §9.
...
7. **Playwright (or Vitest + screenshot harness) compares** key screens to
   `docs/design-refs/` when refs exist.
8. **`aubr check` / unit+integration tests must pass with no Clerk keys and no
   Convex URL** when `VITE_EH_DATA=fixture`.
...
If Slice 1 (`bc-c7fa884b`) wired components directly to Clerk/Convex, next
agent:
...
Starter-kit `/login`, `/signup`, `/dashboard` may remain for kit health; Event
Horizon kid loop does not depend on them overnight.

### Auth model

- **Parent** = Clerk user (morning) **or** fixture parent session (overnight).
- **Kid** = **Convex row** (or fixture row) — **not** a Clerk user.
- Active kid id lives in app session state after parent selects / onboarding
  creates.
- Fixture path: auto parent + auto kid → land **`/hub`**.
- Parent surfaces require PIN (`1234` fixture; hashed PIN morning with
  `PIN_PEPPER`).
...
Normative tables for morning / `convex-test`. Field names may use camelCase in
TS; keep indexes as specified.
...
**Indexes:** `by_missionId` (`missionId`), `by_missionId_order` (`missionId`,
`order`)
...
**Indexes:** `by_kidId` (`kidId`), `by_kid_mission` (`kidId`, `missionId`),
`by_kid_status` (`kidId`, `status`)
...
**Indexes:** `by_attemptId` (`attemptId`), `by_attempt_question` (`attemptId`,
`questionKey`)
...
All pure logic lives under `lib/eh/pure/` (or equivalent) and is unit-tested
without Convex/Clerk.
...
- **+20 XP** profile-level once per local day (first completed mission that
  day).
...
- **Q4:** `anyOf` evidence `[s5, s6]` **AND** MC choice `B` must both be
  correct.
...
Wrong answer → stay on question; Hint available; never auto-reveal the answer
letter on steps 1–2.
...
Hints **never** state the answer letter or exact sentence id on steps 1–2. Step
4 may highlight location but still requires the kid’s action.
...
- Convex action `hints:generate` (name flexible) calls xAI only when
  `XAI_API_KEY` present.
- System rules: age-appropriate; never give the answer; short; push back to
  text/evidence; respect step intent.
...
Sentence / prompt / choice / hint text below is **verbatim** from
`docs/missions/mission-01.md`.
...
**Note on hint step 4 wording:** Product UI may implement glow by
`evidenceSentenceId` (`s3`, `s4`, `s5`/`s6`, `s8`) even when kid-facing copy
avoids saying the id aloud. Do not renumber sentence ids without migrating
attempts.
...
**Handoff note:** If Slice 1 assumed live Clerk, **Slice 2’s first commit** adds
`EhData` + `fixtureAdapter` and redirects UI — do not thrash Slice 1 visuals.
...
4. If `403` on merge to `main`, **stack** the next PR onto the previous slice
   branch and keep the stack green.
5. Cursor bot previously merged docs PRs **#1** and **#2** — do not reopen
   those; continue numbering forward.
6. Do not land app code in this docs PR except optional README one-liner (§ this
   file + README link only for the docs land).
...
| `VITE_EH_DATA_MODE=convex` (alias `VITE_EH_DATA=convex`) | flip off fixtures                          |
...
6. Set `VITE_EH_DATA_MODE=convex` in `.env.local` (alias `VITE_EH_DATA=convex`;
   flip off fixture default)
7. `aubr dev`
8. iPad: Add to Home Screen → open → run **Mission 1** end-to-end (Check + Hint
   gold + debrief)
9. Optional verify: unset `XAI_API_KEY` → static hints still work; visual
   compare vs `docs/design-refs/`; parent PIN (not `1234` in prod — set real PIN
   through parent gate)
...
- [ ] Slices 1–6 implemented per §10 **or** clearly stacked as PRs with Slice 1
      not double-started
...
Starter-kit files outside `lib/eh`, EH routes, and EH components stay unless a
slice must touch theme/providers for fixture auth skip.

---

## 17. Convex API sketch

Morning / `convexAdapter` mapping. Names normative enough to implement without
redesign.
...
All kid-facing reads/writes from UI go through `EhData` — **not** raw hooks in
...
**This docs PR:** land this file (+ optional README link). No app implementation
in the docs-only change.
...
2. If live Clerk was assumed → add `EhData` + `fixtureAdapter` immediately
   (`VITE_EH_DATA=fixture` default)
...
**Binding reminders:** fixtures default · Morning Mac checklist §13 · Mission 1
seed §9 · Slices 1–6 §10 · do-not-reopen fixture rules 21–24.
